...
\quad\text{where}\quad \text{area} = (\sqrt{\text{area}})^2 .
\]
...
\qquad
\Delta K \;=\; Y\,\Delta\sigma_{\text{MPa}}\sqrt{\pi\,a}.
\]
...
> **Units note:** Use \(\Delta K\) in MPa\(\sqrt{\text{m}}\).
> Choose \(C, m\) consistent with those units (e.g., \(C\sim 10^{-12}\), \(m\approx 3\) as a starting point).
> Convert stresses \(\Delta\sigma\) from Pa to MPa when computing \(\Delta K\).

---

### Monte-Carlo workflow

Randomize pore **\(\sqrt{\text{area}}\)**, **aspect ratio**, **surface proximity**, and **depth** to create a specimen set; compute \(N_f\) for each to obtain **scatter**, **median**, and **percentile bands**.
